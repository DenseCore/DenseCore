...
void example_int4_gemm() {
  ...
  std::cout << "Matrix dimensions: C[" << M << "×" << N << "] = "
            << "A[" << M << "×" << K << "] * W[" << N << "×" << K << "]^T"
            << std::endl;
  std::cout << "Quantization: INT4 block-wise, group_size=" << group_size
            << std::endl;
  std::cout << std::endl;
  ...
  for (int n = 0; n < N; n++) {
    for (int g = 0; g < num_groups; g++) {
      ...
      scales[n * num_groups + g] =
          0.01f + static_cast<float>(rand()) / RAND_MAX * 0.05f;
  ...
  std::cout << "Memory saved: " << (N * K * sizeof(float) - W_int4.size()) / 1e6
            << " MB vs FP32" << std::endl;
}
...
void example_layer_integration() {
  ...
  std::cout << "  Input shape: [" << seq_len << ", " << hidden_dim << "]"
            << std::endl;
  std::cout << "  Weight shape: [" << hidden_dim << ", " << hidden_dim
            << "] (INT4)" << std::endl;
  std::cout << std::endl;

  // Perform GEMM
  GemmInt4Fp32_AVX512(output.data(), input.data(), weight_int4.data(),
  ...
  std::cout << "  Output shape: [" << seq_len << ", " << hidden_dim << "]"
            << std::endl;
}
...
void example_performance_comparison() {
  ...
  const long metadata_size =
      (7L * 1000 * 1000 * 1000 / 128) * 8; // scales + zeros

  std::cout << "Memory usage:" << std::endl;
  std::cout << "  FP32:  " << model_size_fp32 / 1e9 << " GB" << std::endl;
  std::cout << "  INT4:  " << (model_size_int4 + metadata_size) / 1e9 << " GB"
            << std::endl;
  std::cout << "  Savings: "
            << ((1.0 -
                 (double)(model_size_int4 + metadata_size) / model_size_fp32) *
                100)
            << "%" << std::endl;
  std::cout << std::endl;
...
int main() {
  ...
  if (level < SimdLevel::AVX512) {
    std::cout
        << "⚠ Warning: AVX512 not available. Using fallback implementation."
        << std::endl;
    std::cout << std::endl;
  }
