...
private:
    ...
    void EnsureInitialized() {
        ...
        std::cerr << "[DEBUG] StaticThreadPool::EnsureInitialized: Creating " << num_threads_
                  << " threads..." << std::endl;
        ...
        std::cerr << "[DEBUG] StaticThreadPool::EnsureInitialized: Done." << std::endl;
        ...
        HardwareTopology::GetInstance().PinThreadPool(workers_, numa_node, PinningPolicy::SCATTER);
    ...
    void WorkerLoop(int thread_id) {
        ...
        while (true) {
            ...
            if (is_gemv_work_) {
                // GEMV path
                simd::GemvParallel(gemv_output_, gemv_input_, gemv_weight_, gemv_K_, gemv_N_,
                                   thread_id, num_threads_);
            } else if (current_work_fn_) {
                // Generic parallel_for path
                const int work_per_thread = (total_work_ + num_threads_ - 1) / num_threads_;
...
CpuBackend::CpuBackend() {
    ...
    std::cout << "[CpuBackend] Initialized with SIMD level: " << selected_isa_ << std::endl;
}
...
void CpuBackend::BeginCapture() {
    if (capturing_) {
        std::cerr << "[CpuBackend] Warning: Already capturing, resetting graph" << std::endl;
    }
    capturing_ = true;
...
std::unique_ptr<OperationGraph> CpuBackend::EndCapture() {
    if (!capturing_) {
        std::cerr << "[CpuBackend] Warning: EndCapture called without BeginCapture" << std::endl;
...
void CpuBackend::ExecuteGraph(const OperationGraph& graph) {
    ...
    for (size_t i = 0; i < graph.NodeCount(); ++i) {
        ...
        // =========================================================================
        default:
            std::cerr << "[CpuBackend] ExecuteGraph: Unsupported op type '" << OpTypeName(node.op)
                      << "' at node " << i << std::endl;
            break;
        }
    }
...
void CpuBackend::Quantize(const Tensor& src, QuantizedTensorView* dst, QuantType type) {
    ...
    case QuantType::Q4_K: {
        ...
        std::cerr << "[CpuBackend] Quantize: Q4 quantization not fully implemented, "
                  << "use GGML model loader for pre-quantized models" << std::endl;
        break;
    }

    default:
        std::cerr << "[CpuBackend] Quantize: Unsupported type " << QuantTypeName(type) << std::endl;
        break;
    }
}
...
void CpuBackend::Dequantize(const QuantizedTensorView& src, Tensor* dst) {
    ...
    default:
        std::cerr << "[CpuBackend] Dequantize: Unsupported type " << QuantTypeName(src.type)
                  << std::endl;
        break;
    }
}
...
void CpuBackend::MatMul(const Tensor& A, const Tensor& B, Tensor* C) {
    ...
    if (M == 1) {
        ...
        constexpr int BLOCK_K = 64;  // Tile size for K dimension (larger for cache line efficiency)
...
void CpuBackend::GemmInt4(const Tensor& A, const Tensor& W, const Tensor& scales,
    ...
    if (M == 1) {
        ...
        if (n_threads <= 1) {
            ...
            kernels::GemvInt4(c_data, a_data, w_data, scales_data, zeros_data, K, N, group_size, 0,
                              N);
        } else {
            // Multi-threaded: partition N across threads using ParallelFor
            StaticThreadPool::Instance().ParallelFor(N, [&](int n_start, int n_end, int thread_id) {
                (void)thread_id;
                kernels::GemvInt4(c_data, a_data, w_data, scales_data, zeros_data, K, N, group_size,
                                  n_start, n_end);
            });
        }
        return;
    ...
    if (OpsRegistry::IsInitialized()) {
        OpsRegistry::Instance().GemmInt4(c_data, a_data, w_data, scales_data, zeros_data, M, N, K,
                                         group_size);
    } else {
#if defined(__AVX512F__)
        simd::GemmInt4Fp32_AVX512(c_data, a_data, w_data, scales_data, zeros_data, M, N, K,
                                  group_size);
#elif defined(__AVX2__)
        simd::GemmInt4Fp32_AVX2(c_data, a_data, w_data, scales_data, zeros_data, M, N, K,
                                group_size);
#else
...
void CpuBackend::AddRMSNorm(const Tensor& input, const Tensor& residual, const Tensor& weight,
...
void CpuBackend::RoPE(const Tensor& input, const Tensor& cos_sin, const int* positions,
...
void CpuBackend::FusedQKVProjection(const Tensor& input, const Tensor& wq, const Tensor& wk,
...
void CpuBackend::FlashAttention(const Tensor& Q, const Tensor& K, const Tensor& V, Tensor* output,
    ...
    FlashAttentionGQA(q_data, k_data, v_data, o_data, batch, n_head, n_head_kv, seq_q, seq_kv,
                      head_dim, config);
}
