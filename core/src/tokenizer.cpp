 ...
 */
static std::vector<std::string> MergeWithPriorityQueue(const TransformerModel* model,
...
std::vector<int> Tokenizer::Tokenize(const TransformerModel* model, const std::string& text,
        ...
        if (it != model->token_to_id.end()) {
            ...
            for (size_t i = 0; i < tok.size(); i++) {
                ...
                if (c_it != model->token_to_id.end()) {
                    ...
                    char buf[8];
                    snprintf(buf, sizeof(buf), "<0x%02X>", static_cast<unsigned char>(tok[i]));
...
std::string Tokenizer::Detokenize(const TransformerModel* model, int token_id) {
    ...
    for (size_t i = 0; i < token.size(); i++) {
        ...
        else if (i + 1 < token.size() && static_cast<unsigned char>(token[i]) == 0xC4 &&
                 static_cast<unsigned char>(token[i + 1]) == 0x8A) {
            result += '\n';
            i++;
        }
        // Check for ▁ (U+2581 = 0xE2 0x96 0x81) -> space (SentencePiece style)
        else if (i + 2 < token.size() && static_cast<unsigned char>(token[i]) == 0xE2 &&
