...
void InferenceContext::Init(size_t buffer_size) {
  ...
  if (ctx_compute) {
    initialized = true;
    std::cerr << "[InferenceContext] Initialized with "
              << (buffer_size / (1024 * 1024)) << " MB persistent buffer"
              << std::endl;
  } else {
    std::cerr << "[InferenceContext] ERROR: Failed to initialize GGML context!"
              << std::endl;
  }
}
...
void SetCurrentBatch(const BatchSpec *batch) { g_batch_context.batch = batch; }

// Get current batch from global context (used in callbacks)
inline const BatchSpec *GetCurrentBatch() { return g_batch_context.batch; }
...
struct KVUpdateGatherUserData {
  ...
  struct ggml_tensor
      *src_tensor; // Pointer to Kcur/Vcur tensor (data accessed at runtime)
};

// Thread-local pool for KVUpdateGatherUserData
static constexpr int kMaxKVUpdateGatherSlots = 256;
static thread_local KVUpdateGatherUserData
    g_kv_update_gather_pool[kMaxKVUpdateGatherSlots];
 ...
 */
void cb_kv_update_and_gather(struct ggml_tensor *dst,
                             const struct ggml_tensor * /* src - unused */,
                             int ith, int nth, void *userdata) {
  ...
  for (int i = 0; i < N; i++) {
    ...
    if (seq_id < 0 ||
        seq_id >= static_cast<int>(ud->batch->block_tables.size()))
      continue;
    ...
    if (logical_block < 0 ||
        logical_block >= static_cast<int>(block_table.size()))
      continue;
  ...
  if (n_past > 0) {
    ...
    if (seq_id >= 0 &&
      ...
      for (int i = 0; i < n_past; i++) {
        ...
        float *dst_slot =
            reinterpret_cast<float *>(dst->data) + i * head_block_size;
  ...
  // ===========================================================================
  float *dst_current =
      reinterpret_cast<float *>(dst->data) + n_past * head_block_size;
...
struct GemvUserData {
  ...
  ggml_type
      input_quant_type; // Quantization type for input (Q8_K, Q8_0, or F32)
...
void cb_gemv_parallel(struct ggml_tensor *dst, const struct ggml_tensor *src,
  ...
  if (quant_input && ith == 0 && ud->input_quant_type != GGML_TYPE_F32) {
    const auto *input_type_traits =
        ggml_get_type_traits_cpu(ud->input_quant_type);
  ...
  if (quant_input && type_traits_cpu && type_traits_cpu->vec_dot) {
    for (int k = k_start; k < k_end; k++) {
      const void *row_ptr =
          reinterpret_cast<const char *>(weight_data) + k * row_stride;
  ...
  if (!type_traits || !type_traits->to_float ||
      N > static_cast<int>(kMaxDequantBufferSize)) {
    for (int k = k_start; k < k_end; k++)
      output[k] = 0.0f;
    return;
  }
  ...
  if (ggml_is_quantized(wtype)) {
    ...
    if (type_traits_cpu && type_traits_cpu->vec_dot) {
      ...
      if (input_type_traits && input_type_traits->from_float) {
        ...
        if (quant_input_size > 0 &&
            quant_input_size <= kMaxQuantInputBufferSize) {
...
struct ggml_tensor *BuildTransformerGraph(
  ...
  for (int il = 0; il < n_layer; ++il) {
    ...
    if (use_cache) { // Re-enabled old KV cache approach
      ...
      *k_ud = {cache, il, head_dim_kv,
               true}; // batch accessed via GetCurrentBatch()
      KVCacheUserData *v_ud = GetKVCacheUserData(il, false);
      *v_ud = {cache, il, head_dim_kv,
               false}; // batch accessed via GetCurrentBatch()
    ...
    if (false) { // DISABLED: New approach has graph dependency bugs
      ...
      struct ggml_tensor *K_all_tensor = ggml_new_tensor_3d(
          ctx_c, GGML_TYPE_F32, head_dim_kv, n_head_kv, n_total_tokens);
      struct ggml_tensor *V_all_tensor = ggml_new_tensor_3d(
          ctx_c, GGML_TYPE_F32, head_dim_kv, n_head_kv, n_total_tokens);
      ...
      KVUpdateGatherUserData *v_gather_ud =
          GetKVUpdateGatherUserData(il, false);
      ...
      K_all = ggml_map_custom1(ctx_c, K_all_tensor, cb_kv_update_and_gather, 1,
                               k_gather_ud);
      V_all = ggml_map_custom1(ctx_c, V_all_tensor, cb_kv_update_and_gather, 1,
                               v_gather_ud);
...
// Runtime alignment check (debug builds only)
        ...
        if (row_bytes < GGML_MEM_ALIGN) {
          std::cerr << "[DenseCore] WARNING: head_dim=" << head_dim_kv
                    << " may cause unaligned SIMD access (row_bytes="
                    << row_bytes << ", GGML_MEM_ALIGN=" << GGML_MEM_ALIGN << ")"
                    << std::endl;
        }
#endif

        // Reshape to 3D for permutation
        struct ggml_tensor *Q_r =
            ggml_reshape_3d(ctx_c, Qcur, head_dim_q, n_head, N);
        ...
        struct ggml_tensor *K_perm = ggml_permute(
            ctx_c, K_r, 0, 2, 1, 3); // [head_dim, n_total, n_head_kv]
        struct ggml_tensor *V_perm = ggml_permute(
            ctx_c, V_r, 0, 2, 1, 3); // [head_dim, n_total, n_head_kv]
        ...
        for (int h_kv = 0; h_kv < n_head_kv; ++h_kv) {
          ...
          struct ggml_tensor *K_head =
              ggml_view_2d(ctx_c, K_perm, head_dim_kv, n_total_tokens,
                           K_perm->nb[1], k_offset);
          struct ggml_tensor *V_head =
              ggml_view_2d(ctx_c, V_perm, head_dim_kv, n_total_tokens,
                           V_perm->nb[1], v_offset);
        ...
        struct ggml_tensor *V_t = ggml_permute(
            ctx_c, V_fa, 1, 0, 2, 3); // [n_total, head_dim, n_head]
        V_t = ggml_cont(ctx_c, V_t);
