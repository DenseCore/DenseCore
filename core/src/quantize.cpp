...
int QuantizeGGML(const char* input_path, const char* output_path, const QuantConfig& config) {
    ...
    std::cout << "[Quantize] Model loaded. Quantizing to " << config.GetFormatName() << "..."
              << std::endl;
    ...
    for (int i = 0; i < n_kv; ++i) {
        ...
        if (type == GGUF_TYPE_ARRAY) {
            ...
            case GGUF_TYPE_BOOL:
                ...
                break;
            default:
                std::cerr << "Warning: Skipping unknown KV type for key: " << key << std::endl;
            }
        }
    }
    ...
    for (int i = 0; i < n_tensors; ++i) {
        ...
        if (quantize) {
            ...
            std::cout << "  Converted " << name << " [" << tensor->ne[0] << "x" << tensor->ne[1]
                      << "]" << std::endl;
        } else {
...
int QuantizeINT4Custom(const char* input_path, const char* output_path, const QuantConfig& config) {
    ...
    for (int i = 0; i < n_tensors; ++i) {
        ...
        if (is_quantized[i] && IsINT4Quantized(tensor)) {
            ...
            struct ggml_tensor* qw = ggml_new_tensor_1d(ctx_in, GGML_TYPE_I8, packed_size);
            ...
            struct ggml_tensor* scales_t = ggml_new_tensor_1d(ctx_in, GGML_TYPE_F32, meta_size);
            ...
            struct ggml_tensor* zeros_t = ggml_new_tensor_1d(ctx_in, GGML_TYPE_F32, meta_size);
    ...
    std::cout << std::endl;
    std::cout << "[Quantize] Done! Quantized " << quantized_count << "/" << n_tensors
              << " tensors to INT4_BLOCKWISE." << std::endl;
...
void PrintUsage(const char* prog) {
    std::cerr << "Usage: " << prog << " <input.gguf> <output.gguf> [type] [block_size]"
              << std::endl;
    std::cerr << std::endl;
