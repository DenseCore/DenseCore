...
HybridScheduler::HybridScheduler(const SchedulerConfig& config) : HybridScheduler() {
    impl_->config = config;
}
...
bool HybridScheduler::ProfileModel(const ModelConfig& model_config) {
    ...
    for (int layer = 0; layer < model_config.n_layers; ++layer) {
        ...
        // Profile attention operations
        {
            ...
            qkv_prof.recommended_unit = impl_->SelectBestUnit(qkv_prof.op_type, M, K, N, false);
            qkv_prof.expected_latency_us = std::min(
                {qkv_prof.cpu_latency_us, qkv_prof.gpu_latency_us, qkv_prof.ane_latency_us});
        ...
        // Profile FFN operations
        {
            ...
            ffn_prof.recommended_unit = impl_->SelectBestUnit(ffn_prof.op_type, M, K, N, false);
            ffn_prof.expected_latency_us = std::min(
                {ffn_prof.cpu_latency_us, ffn_prof.gpu_latency_us, ffn_prof.ane_latency_us});
...
ExecutionPlan HybridScheduler::GetExecutionPlan(int batch_size, int seq_len, bool is_prefill) {
    ...
    for (const auto& layer_profile : impl_->layerProfiles) {
        for (const auto& op_profile : layer_profile.operations) {
            ...
            task.unit = is_prefill ? (impl_->gpuBackend ? ComputeUnit::GPU : ComputeUnit::CPU)
...
void HybridScheduler::AdaptToThermalState() {
    ...
    case apple::ThermalState::Serious:
        ...
        std::cout << "[HybridScheduler] Thermal Serious: Enabling efficiency mode" << std::endl;
        break;
