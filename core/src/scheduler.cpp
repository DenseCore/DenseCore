...
void Scheduler::ScheduleWaiting(SchedulerOutput& output) {
    ...
    for (auto& group : sorted_queue) {
        ...
        if (block_manager_->GetFreeBlockCount() < blocks_needed) {
            std::cerr << "[TRACE] Scheduler: Not enough blocks for seq " << seq_id << " (needed "
                      << blocks_needed << ", free " << block_manager_->GetFreeBlockCount() << ")"
                      << std::endl;
            still_waiting.push_back(group);
            continue;
        }
        ...
        if (new_blocks.empty()) {
            std::cerr << "[TRACE] Scheduler: Allocate failed for seq " << seq_id << std::endl;
            still_waiting.push_back(group);
            continue;
        }

        std::cerr << "[TRACE] Scheduler: Scheduled seq " << seq_id << " (prefill)" << std::endl;
...
void PrintSchedulerDebugInfo(const Scheduler& scheduler) {
    ...
    std::cout << "[Scheduler] Waiting: " << stats.waiting_count
              << " Running: " << stats.running_count << " Swapped: " << stats.swapped_count
