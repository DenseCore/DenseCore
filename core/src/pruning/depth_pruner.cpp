...
std::vector<float>
DepthPruner::ComputeImportanceScores(const TransformerModel &model) {
  std::vector<float> scores;
  scores.reserve(model.layers.size());

  std::cout << "[Pruner] Computing importance scores for "
            << model.layers.size() << " layers..." << std::endl;
...
std::vector<int>
DepthPruner::SelectLayersToRemove(const std::vector<float> &scores,
                                  int target_count) {
  ...
  if (layers_to_remove <= 0) {
    std::cout << "[Pruner] No layers to remove (target >= original)"
              << std::endl;
    return {};
  }
  ...
  std::vector<int> to_remove(indices.begin(),
                             indices.begin() + layers_to_remove);

  // Sort in descending order for safe removal (remove from end first)
  std::sort(to_remove.rbegin(), to_remove.rend());

  std::cout << "[Pruner] Selected " << to_remove.size()
            << " layers to remove (least important)" << std::endl;
...
void DepthPruner::RemoveLayers(TransformerModel *model,
  ...
  std::cout << "[Pruner] Updated n_layer: " << model->hparams.n_layer
            << std::endl;
}
...
void DepthPruner::PruneModel(TransformerModel *model) {
  ...
  stats_.pruned_hidden_size =
      model->hparams.n_embd;  // Unchanged for depth pruning
  ...
  std::cout << "  Reduction: " << (stats_.GetLayerReduction() * 100) << "%"
            << std::endl;
}
