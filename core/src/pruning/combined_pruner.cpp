...
std::vector<float>
CombinedPruner::ComputeImportanceScores(const TransformerModel &model) {
...
void CombinedPruner::PruneModel(TransformerModel *model) {
  ...
  if (config_.target_n_layer > 0 &&
      config_.target_n_layer < (int)model->layers.size()) {
    ...
    std::cout << "[CombinedPruner] Depth phase complete: "
              << depth_stats_.layers_before << " -> "
  ...
  if (config_.target_hidden_size > 0 &&
      config_.target_hidden_size < (int)model->hparams.n_embd) {
    ...
    std::cout << "[CombinedPruner] Width phase complete: "
              << width_stats_.hidden_before << " -> "
  ...
  std::cout << "========================================" << std::endl;
  std::cout << "  Layers: " << stats_.original_n_layer << " -> "
            << stats_.pruned_n_layer << " ("
            << (100.0f * stats_.GetLayerReduction()) << "% reduction)"
            << std::endl;
  std::cout << "  Hidden: " << stats_.original_hidden_size << " -> "
            << stats_.pruned_hidden_size << " ("
            << (100.0f * stats_.GetWidthReduction()) << "% reduction)"
            << std::endl;

  // Estimate total compression
  float layer_factor = (float)stats_.pruned_n_layer / stats_.original_n_layer;
  float width_factor =
      (float)stats_.pruned_hidden_size / stats_.original_hidden_size;
  ...
  std::cout << "  Estimated param reduction: "
            << (1.0f - estimated_compression) * 100 << "%" << std::endl;
}
