...
void AWQQuantizer::Calibrate(const std::vector<float *> &samples,
                             size_t sample_size) {
  ...
  calibrated_ = true;
  std::cout << "[AWQQuantizer] Calibrated with " << samples.size()
            << " samples, " << sample_size << " channels" << std::endl;
}
...
void AWQQuantizer::ApplyChannelScaling(float *weights, size_t rows,
                                       size_t cols) {
...
void AWQQuantizer::QuantizeWeight(struct ggml_tensor *tensor) {
  ...
  if (tensor->type == target_type) {
    std::cout << "[AWQQuantizer] Already quantized: " << tensor->name
              << std::endl;
    return;
  }
  ...
  if (calibrated_ && !channel_scales_.empty()) {
    ...
    std::cout << "[AWQQuantizer] Applied AWQ scaling to " << tensor->name
              << std::endl;
  }
  ...
  std::cout << "[AWQQuantizer] Quantized " << tensor->name << " to "
            << type_name << " (" << nelements << " elements, " << quant_size
            << " bytes)" << (calibrated_ ? " [AWQ scaled]" : "") << std::endl;
}
