...
void SymmetricInt8Quantizer::Calibrate(const std::vector<float *> &samples,
                                       size_t sample_size) {
...
void SymmetricInt8Quantizer::QuantizeWeight(struct ggml_tensor *tensor) {
  if (!ShouldQuantize(tensor->name)) {
    std::cout << "[SymmetricInt8Quantizer] Skipping: " << tensor->name
              << std::endl;
    return;
  }
  ...
  if (tensor->type == target_type) {
    std::cout << "[SymmetricInt8Quantizer] Already quantized: " << tensor->name
              << std::endl;
    return;
  }
  ...
  if (tensor->type != GGML_TYPE_F32 && tensor->type != GGML_TYPE_F16) {
    std::cout << "[SymmetricInt8Quantizer] Skipping non-FP tensor: "
              << tensor->name << " (type=" << tensor->type << ")" << std::endl;
    return;
  }
  ...
  if (tensor->type == GGML_TYPE_F32) {
    ...
    std::cerr << "[SymmetricInt8Quantizer] Unsupported source type"
              << std::endl;
    return;
  }
  ...
  ggml_quantize_chunk(target_type, src_data, quant_buffer.data(), 0, nelements,
                      tensor->ne[0], nullptr);
  ...
  // Log success
  std::cout << "[SymmetricInt8Quantizer] Quantized " << tensor->name
            << " to Q8_0 (" << nelements << " elements, " << q8_size
            << " bytes)" << std::endl;
}
