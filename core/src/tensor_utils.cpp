...
struct ggml_tensor *SliceTensor(struct ggml_context *ctx,
  ...
  if (!ctx || !src || indices.empty()) {
    std::cerr << "[TensorUtils] Error: Invalid arguments to SliceTensor"
              << std::endl;
    return nullptr;
  }
  ...
  if (src->type != GGML_TYPE_F32 && src->type != GGML_TYPE_F16) {
    std::cerr << "[TensorUtils] Error: SliceTensor only supports F32/F16. "
              << "Got type " << src->type << ". Prune before quantization."
              << std::endl;
    return nullptr;
  }
  ...
  struct ggml_tensor *dst =
      ggml_new_tensor_2d(ctx, src->type, new_rows, new_cols);
  if (!dst) {
...
struct ggml_tensor *Slice1DTensor(struct ggml_context *ctx,
  ...
  if (!ctx || !src || indices.empty()) {
    std::cerr << "[TensorUtils] Error: Invalid arguments to Slice1DTensor"
              << std::endl;
    return nullptr;
  }
  ...
  if (!dst) {
    std::cerr << "[TensorUtils] Error: Failed to allocate 1D tensor of size "
              << new_size << std::endl;
    return nullptr;
  }
