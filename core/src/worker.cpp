...
void EngineLoop(EngineState* state) {
    try {
        ...
        if (!model_entry) {
            std::cerr << "[DenseCore] FATAL: No model loaded. EngineLoop exiting." << std::endl;
            return;
        }
        ...
        // =========================================================================
        {
            ...
            std::cout << "[DenseCore] Orchestration thread unpinned (OS Scheduled). "
                      << "NUMA nodes detected: " << topo.GetNumaNodeCount() << std::endl;
        ...
        while (state->status != EngineStatus::STOPPED) {
                ...
                while (state->status == EngineStatus::RUNNING) {
                    ...
                    if (req->tokens.empty()) {
                        std::cerr << "[DenseCore] FATAL: Request " << req->id
                                  << " has no tokens (should be pre-tokenized)" << std::endl;
                        req->finished = true;
                        state->metrics.failed_requests++;
                        // Push error event to callback queue (instead of direct callback)
                        if (req->callback) {
                            PushResultEvent(state, req->id, "Error: Missing tokens", true, true,
                    ...
                    if (seq_id < 0) {
                        ...
                        // Push error event to callback queue (instead of direct callback)
                        if (req->callback) {
                            PushResultEvent(state, req->id, "Error: Scheduler queue full", true,
                    ...
                    std::cerr << "[TRACE] Request " << req->id << " moved to active" << std::endl;
                }
            }
            ...
            // 2. Process cancelled/finished requests first
            {
                ...
                while (it != state->active_requests.end()) {
                    ...
                    if (req->cancelled && !req->finished) {
                        ...
                        // callback)
                        if (req->callback) {
                            PushResultEvent(state, req->id, "Error: Request cancelled", true, true,
                                            req->callback, req->user_data);
                        }
            ...
            for (int preempted_seq_id : sched_output.preempted_seq_ids) {
                ...
                if (req_it != seq_to_request.end()) {
                    ...
                    // re-admission
                    LOG_INFO("Preempting request ", req->id, " (seq_id=", preempted_seq_id, ")");
                    current_kv_cache->block_manager->Free(req->block_table);
                    req->block_table.clear();
            ...
            for (int seq_id : sched_output.prefill_seq_ids) {
                ...
                Request* req = req_it->second;
                std::cerr << "[TRACE] Prefill loop: Found request " << req->id << " for seq "
                          << seq_id << " (finished=" << req->finished << ")" << std::endl;
                if (req->finished)
                    continue;
            ...
            for (int seq_id : sched_output.decode_seq_ids) {
                ...
                Request* req = req_it->second;
                std::cerr << "[TRACE] Decode loop: Found request " << req->id << " for seq "
                          << seq_id << " (finished=" << req->finished << ")" << std::endl;
                if (req->finished)
                    continue;
            ...
            if (!ctx_nodes) {
                std::cerr << "[DenseCore] FATAL: InferenceContext not initialized!" << std::endl;
                continue;
            }
            ...
            if (!output || !embd_inp || !pos) {
                std::cerr << "Fatal: Content creation failed or tensors missing" << std::endl;
                continue;  // Recover
            }
            ...
            std::cerr << "[TRACE] Graph compute done for batch size " << batch.num_seqs
                      << std::endl;
            ...
            for (int i = 0; i < batch.num_seqs; ++i) {
                ...
                std::cerr << "[DEBUG] EngineLoop: Processing output for request " << req->id
                          << std::endl;
                ...
                if (is_embedding_batch) {
                    ...
                    std::string token_str = Tokenizer::Detokenize(current_model, best_token);
                    ...
                    if (req->callback) {
                        ...
                        PushResultEvent(state, req->id, token_str, false, false, req->callback,
                                        req->user_data);
                    }
                    ...
                    while ((int)req->block_table.size() < blocks_needed) {
                        ...
                        if (new_blocks.empty()) {
                            std::cerr << "[DenseCore] OOM during generation for req " << req->id
                                      << std::endl;
                            req->finished = true;
                            ...
                            if (req->callback) {
                                PushResultEvent(state, req->id, "Error: Out of memory", true, true,
                                                req->callback, req->user_data);
                            }
                            break;
                        }
