...
void EngineLoop(EngineState *state) {
  ...
  // Initialize persistent compute buffer (eliminates malloc/free per iteration)
  ...
  while (state->status != EngineStatus::STOPPED) {
    // 1. Fetch new requests and register with scheduler
    {
      ...
      while (state->status == EngineStatus::RUNNING &&
        ...
        // Tokens are pre-tokenized in SubmitRequest - assert this invariant
        if (req->tokens.empty()) {
        ...
        int seq_id = state->scheduler->AddRequest(req->id, req->tokens.size(),
                                                  req->max_tokens,
    ...
    // The compute buffer is reused across iterations to avoid malloc overhead
    // Graph Caching Logic
    ...
    // Determine if cacheable: only strictly consistent batches (e.g. Decode
    // with 1 token/seq) For MVP we cache by num_seqs. We verify if input size
    // matches expected 1 token/seq for decode.
    bool cacheable =
        (!is_embedding_batch && batch.tokens.size() == (size_t)batch.num_seqs);
    ...
    // RAII guard for temporary context - declared here so it lives through
    // compute For cached graphs: this stays nullptr (no-op) For non-cached
    // graphs: holds the temp context until end of loop iteration
    ...
    // ctx_temp_guard destructor will call ggml_free() at end of loop iteration
    ...
    for (int i = 0; i < batch.num_seqs; ++i) {
      ...
      if (is_embedding_batch) {
        ...
        std::string token_str = "[TID:" + std::to_string(best_token) + "]";

        if (req->json_mode) {
          std::string decoded =
              Tokenizer::Detokenize(current_model, best_token);
          req->grammar.UpdateState(decoded);
        }
