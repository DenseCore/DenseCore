...
void KVCache::RemoveLast(int n) { n_tokens = std::max(0, n_tokens - n); }
...
KVCache *InitKVCache(TransformerModel *model) {
  ...
  size_t cache_size = (size_t)cache->n_layer * cache->n_ctx *
                      model->hparams.n_head_kv * cache->head_dim *
                      sizeof(float) * 2;
  ...
  cache->k = ggml_new_tensor_4d(cache->ctx, GGML_TYPE_F32, cache->head_dim,
                                model->hparams.n_head_kv, cache->n_ctx,
                                cache->n_layer);
...
void PagedKVCache::CopyBlockDataLayer(int src_block_id, int dst_block_id,
                                      int layer) {
  if (src_block_id == dst_block_id)
    return;
  if (src_block_id < 0 || dst_block_id < 0)
...
PagedKVCache *InitPagedKVCache(TransformerModel *model, int max_num_seqs,
  ...
  if (type != GGML_TYPE_F32 && type != GGML_TYPE_F16 &&
      type != GGML_TYPE_Q8_0) {
    std::cerr
        << "[KVCache] Warning: Unsupported cache type, falling back to F16"
        << std::endl;
    type = GGML_TYPE_F16;
    cache->cache_type = type;
  }
  ...
  int64_t nelements =
      (int64_t)cache->head_dim * cache->n_head_kv * BLOCK_SIZE * n_layer_blocks;
...
bool PagedKVCache::IsQuantized() const { return cache_type == GGML_TYPE_F16; }

int PagedKVCache::GetElementsPerSlot() const { return head_dim * n_head_kv; }

void PagedKVCache::WriteKSlot(int block_id, int layer, int slot,
                              const float *data) {
...
void PagedKVCache::WriteVSlot(int block_id, int layer, int slot,
                              const float *data) {
...
void PagedKVCache::ReadKSlot(int block_id, int layer, int slot,
                             float *out) const {
  const void *ptr =
      const_cast<PagedKVCache *>(this)->GetKSlotPtr(block_id, layer, slot);
  if (!ptr)
    return;
...
void PagedKVCache::ReadVSlot(int block_id, int layer, int slot,
                             float *out) const {
  const void *ptr =
      const_cast<PagedKVCache *>(this)->GetVSlotPtr(block_id, layer, slot);
  if (!ptr)
    return;
...
void PagedKVCache::WriteKSlots(int block_id, int layer, int start_slot,
                               int num_slots, const float *data) {
  if (num_slots <= 0)
    return;
...
void PagedKVCache::WriteVSlots(int block_id, int layer, int start_slot,
                               int num_slots, const float *data) {
  if (num_slots <= 0)
    return;
...
void PagedKVCache::ReadKSlots(int block_id, int layer, int start_slot,
                              int num_slots, float *out) const {
  if (num_slots <= 0)
    return;
...
void PagedKVCache::ReadVSlots(int block_id, int layer, int start_slot,
                              int num_slots, float *out) const {
  if (num_slots <= 0)
    return;
...
void PagedKVCache::PrefetchBlock(int block_id, int layer) const {
  const void *k_ptr =
      const_cast<PagedKVCache *>(this)->GetKBlockPtr(block_id, layer);
  const void *v_ptr =
      const_cast<PagedKVCache *>(this)->GetVBlockPtr(block_id, layer);
...
void PagedKVCache::PrefetchNextLayer(const std::vector<int> &block_ids,
