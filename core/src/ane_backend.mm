...
void ANEBackend::MatMul(const Tensor& A, const Tensor& B, Tensor* C) {
    ...
    std::cerr << "[ANEBackend] Generic MatMul not implemented. "
              << "Use CompileMatMul/ExecuteMatMul for ANE acceleration." << std::endl;
}
...
void ANEBackend::GemmInt4(const Tensor& A, const Tensor& W, const Tensor& scales,
    ...
    throw std::runtime_error("[ANEBackend] INT4 GEMM not supported on Neural Engine");
}
...
void ANEBackend::FusedQKVProjection(const Tensor& input, const Tensor& wq, const Tensor& wk,
    ...
    std::cerr << "[ANEBackend] FusedQKVProjection not compiled, using fallback" << std::endl;
}
...
bool ANEBackend::ExecuteTransformerLayerDynamic(const std::string& layer_prefix, const float* input,
    ...
    if (status != ANEOpStatus::Ready) {
        std::cerr << "[ANEBackend] Bucketed model '" << model_name
    ...
    bool success = ExecuteTransformerLayer(model_name, padded_in_ptr, padded_out_ptr,
                                           padded_pos_ptr, bucket_size);
    if (!success) {
        return false;
    }
