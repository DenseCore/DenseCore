...
int SubmitEmbeddingRequest(DenseCoreHandle handle, const char* prompt, EmbeddingCallback callback,
                           void* user_data) {
    // Default: MEAN pooling with normalization
    return SubmitEmbeddingRequestEx(handle, prompt, 0, 1, callback, user_data);
}

int SubmitEmbeddingRequestEx(DenseCoreHandle handle, const char* prompt, int pooling_type,
...
int SubmitBatchEmbeddingRequest(DenseCoreHandle handle, const char** prompts, int num_prompts,
    ...
    for (int i = 0; i < num_prompts; i++) {
        int id = SubmitEmbeddingRequestEx(handle, prompts[i], pooling_type, normalize, callback,
                                          user_data);
        if (i == 0)
            first_id = id;
    }
...
DENSECORE_API DenseCoreHandle InitEngine(const char* model_path, const char* /*reserved*/,
    ...
    try {
        ...
        size_t bytes_per_token = (size_t)head_dim * n_head_kv * n_layer * 2 * sizeof(ggml_fp16_t);
        ...
        std::cout << "[DenseCore] Auto-configured max_seq_len: " << optimal_seq_len
                  << " (KV cache: ~" << (bytes_per_token * optimal_seq_len / 1024 / 1024) << " MB, "
        ...
        if (simd_level >= densecore::simd::SimdLevel::AVX512) {
            std::cout << "[DenseCore] Flash Attention Enabled ("
                      << densecore::simd::SimdLevelName(simd_level) << " detected)" << std::endl;
        } else {
        ...
        if (state->numa_node_id >= 0) {
            std::cout << "[DenseCore] NUMA binding: node " << state->numa_node_id << std::endl;
        }

        // Initialize Paged KV Cache with NUMA-aware allocation
        PagedKVCache* cache = InitPagedKVCache(model, 1, optimal_seq_len, GGML_TYPE_F16, -1);
        if (!cache) {
            delete model;
            delete state;
            return nullptr;
        ...
        std::cerr << "[DenseCore] Exception in InitEngine: " << e.what() << std::endl;
        return nullptr;
    } catch (...) {
        ...
        while (true) {
            ResultEvent event;
            {
                ...
                state->result_cv.wait(lock, [state]() {
                    return !state->result_queue.empty() || state->status == EngineStatus::STOPPED;
                });
            ...
            try {
                ...
                std::cerr << "[DenseCore] Exception in callback (request " << event.request_id
                          << "): " << e.what() << std::endl;
            } catch (...) {
...
int SubmitRequestIds(DenseCoreHandle handle, const int* tokens, int n_tokens, int max_tokens,
                     TokenCallback callback, void* user_data) {
...
int SubmitRequestWithFormat(DenseCoreHandle handle, const char* prompt, int max_tokens,
...
int CancelRequest(DenseCoreHandle handle, int request_id) {
    ...
    LOG_INFO("Request ", request_id, " not active - cancellation may be delayed.");
...
DetailedMetrics GetDetailedMetrics(DenseCoreHandle handle) {
    ...
    if (m.completed_requests > 0) {
        m.tokens_per_second = (float)m.total_tokens_generated / std::max(1L, m.completed_requests);
    }
    ...
    // Latency metrics from samples
    {
        ...
        m.p99_queue_wait_time =
            state->metrics.CalculatePercentile(state->metrics.queue_wait_samples, 0.99f);
    }
...
int LoadModel(DenseCoreHandle handle, const char* model_id, const char* model_path,
              int /*threads*/) {
