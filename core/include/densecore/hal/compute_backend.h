...
public:
     ...
     */
    virtual AcceleratorTraits GetTraits() const { return AcceleratorTraits::GenericCPU(); }
     ...
     *       returns true.
     */
    virtual void SynchronizeMemory(void* ptr, size_t size_bytes, MemorySyncDirection direction) {
     ...
     */
    [[deprecated("Use AllocateUnified() for zero-copy UMA access")]]
    virtual void CopyFromDevice(void* dst, const void* src, size_t size_bytes) = 0;
     ...
     */
    virtual void MatMulMixed(const UnifiedTensorRef& input, const UnifiedTensorRef& weight,
                             Tensor* output) {
        ...
        if (input.IsDense() && weight.IsDense()) {
            ...
            throw std::runtime_error("MatMulMixed: Dense x Quantized requires backend override");
        } else {
    ...
    virtual void GemmInt4(const Tensor& A, const Tensor& W, const Tensor& scales,
                          const Tensor& zero_points, Tensor* C, int group_size) = 0;
     ...
     */
    virtual void Quantize(const Tensor& src, QuantizedTensorView* dst, QuantType type) {
     ...
     */
    virtual void RMSNorm(const Tensor& input, const Tensor& weight, Tensor* output,
                         float eps = 1e-5f) = 0;
     ...
     */
    virtual void AddRMSNorm(const Tensor& input, const Tensor& residual, const Tensor& weight,
     ...
     */
    virtual void RoPE(const Tensor& input, const Tensor& cos_sin, const int* positions,
                      Tensor* output, int rope_dim = -1) = 0;
     ...
     */
    virtual void FusedQKVProjection(const Tensor& input, const Tensor& wq, const Tensor& wk,
     ...
     */
    virtual void FlashAttention(const Tensor& Q, const Tensor& K, const Tensor& V, Tensor* output,
                                float scale, bool causal = true, int n_head_kv = -1) = 0;
