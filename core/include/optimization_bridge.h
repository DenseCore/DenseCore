 ...
 */
using RoPE_fn = void (*)(float *out, const float *in, const float *cos_sin,
                         const int *positions, int n_tokens, int head_dim,
                         int rope_dim, int ith, int nth);
 ...
 */
using GemmInt4_fn = void (*)(float *C, const float *A, const uint8_t *W_int4,
 ...
 */
struct Ops {
  static void RoPE(float *out, const float *in, const float *cos_sin,
                   const int *positions, int n_tokens, int head_dim,
                   int rope_dim, int ith = 0, int nth = 1) {
    OpsRegistry::Instance().RoPE(out, in, cos_sin, positions, n_tokens,
                                 head_dim, rope_dim, ith, nth);
  }

  static void GemmInt4(float *C, const float *A, const uint8_t *W_int4,
                       const float *scales, const float *zero_points, int M,
                       int N, int K, int group_size) {
    OpsRegistry::Instance().GemmInt4(C, A, W_int4, scales, zero_points, M, N, K,
                                     group_size);
  }

  static void Softmax(float *data, size_t n) {
    OpsRegistry::Instance().Softmax(data, n);
  }
