...
public:
     ...
     */
    KVBlockAllocator(size_t num_blocks, size_t block_stride, size_t alignment = 64,
                     int numa_node = -1)
        : arena_(nullptr),
          arena_size_(0),
        ...
        if (numa_node >= 0) {
            ...
            if (arena_) {
                ...
                std::cerr << "[KVBlockAllocator] NUMA allocation failed on node " << numa_node
                          << ", falling back to standard allocation" << std::endl;
            }
        }
    ...
    bool Free(int block_id) {
        ...
        if (!allocated_[block_id]) {
            std::cerr << "[KVBlockAllocator] Warning: Double-free detected for block " << block_id
                      << std::endl;
            return false;
        }
     ...
     */
    template <typename T>
    T* GetBlockPtrAs(int block_id) {
        return static_cast<T*>(GetBlockPtr(block_id));
    }

    template <typename T>
    const T* GetBlockPtrAs(int block_id) const {
     ...
     */
    bool IsNumaAllocated() const { return alloc_type_ == BlockAllocationType::Numa; }
