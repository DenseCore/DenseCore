 ...
 */
inline void FlashAttentionForward(const float* Q, const float* K, const float* V, float* O,
    ...
    for (int j = 0; j < seq_len_kv; j += Bc) {
        ...
        for (int i = 0; i < seq_len_q; i += Br) {
            ...
            simd::ComputeQK_AVX512(Q + i * head_dim, K + j * head_dim, scratch.qk_block.data(),
                                   q_len, kv_len, head_dim, scale);
            ...
            simd::SoftmaxBlock_AVX512(scratch.qk_block.data(), block_max, block_sum, q_len, kv_len,
                                      first_kv_block);
            ...
            simd::UpdateOutput_AVX512(O + i * head_dim, scratch.pv_block.data(), alpha_ptr,
                                      beta_ptr, q_len, head_dim);
        }
    }
}
 ...
 */
inline void FlashAttentionBatched(const float* Q, const float* K, const float* V, float* O,
                                  int batch, int n_head, int seq_len_q, int seq_len_kv,
                                  int head_dim, const FlashAttentionConfig& config, int ith = 0,
                                  int nth = 1) {
    ...
    for (int idx = work_start; idx < work_end; idx++) {
        ...
        FlashAttentionForward(q_ptr, k_ptr, v_ptr, o_ptr, seq_len_q, seq_len_kv, head_dim, config,
                              scratch);
    }
}
 ...
 * @param nth Total number of threads for work partitioning
 */
inline void FlashAttentionGQA(const float* Q, const float* K, const float* V, float* O, int batch,
                              int n_head_q, int n_head_kv, int seq_len_q, int seq_len_kv,
                              int head_dim, const FlashAttentionConfig& config, int ith = 0,
                              int nth = 1) {
