...
struct ChannelQuantParams {
  ...
  const int32_t *zero_points =
      nullptr; ///< Zero point per channel [num_channels]
...
struct QuantizationParams {
  ...
  static QuantizationParams PerChannel(const float *scales,
                                       const int32_t *zero_points,
  ...
  static QuantizationParams PerBlock(const void *scales,
...
struct QuantizedTensorView {
   ...
   */
  static QuantizedTensorView MakeINT8(void *data, int64_t rows, int64_t cols,
                                      float scale, int32_t zero_point = 0,
                                      DeviceType device = DeviceType::CPU) {
    QuantizedTensorView v;
    v.data = data;
    v.shape = {rows, cols, 0, 0};
   ...
   */
  static QuantizedTensorView MakeQ4K(void *data, int64_t rows, int64_t cols,
    ...
    int32_t num_blocks =
        BlockQuantParams::CalcNumBlocks(rows * cols, block_size);
    v.quant_params =
        QuantizationParams::PerBlock(scales, zeros, block_size, num_blocks);
    return v;
  }
