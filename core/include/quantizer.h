...
public:
  ...
  virtual void Calibrate(const std::vector<float *> &samples,
                         size_t sample_size) {
  ...
  virtual bool ShouldQuantize(const std::string &tensor_name) const {
    ...
    if (config_.skip_embeddings &&
        tensor_name.find("embeddings") != std::string::npos) {
      return false;
    }
