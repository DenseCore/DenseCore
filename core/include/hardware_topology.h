...
public:
   ...
   */
  bool
  PinCurrentThreadToNumaNode(int numa_node,
   ...
   */
  static bool PinThreadByHandle(std::thread::native_handle_type handle,
                                int core_id) {
#if defined(__linux__) && !defined(__ANDROID__)
    cpu_set_t cpuset;
    CPU_ZERO(&cpuset);
   ...
   */
  void
  ConfigureThreadPoolAffinity(std::function<void(int, int)> thread_init_fn,
                              int num_threads, int numa_node,
                              PinningPolicy policy = PinningPolicy::SCATTER) {
    int target_node = (numa_node >= 0) ? numa_node : 0;
    auto physical_cores = GetPhysicalCoreIds(target_node);
    if (physical_cores.empty())
   ...
   */
  void
  SetupComputeThreadAffinity(int numa_node, int n_threads,
                             PinningPolicy policy = PinningPolicy::SCATTER) {
...
private:
  ...
  void InitializeWithHwloc() {
    ...
    for (int i = 0; i < num_pus; ++i) {
      ...
      hwloc_obj_t core =
          hwloc_get_ancestor_obj_by_type(topology_, HWLOC_OBJ_CORE, pu);
      if (core) {
      ...
      // Find NUMA node
      hwloc_obj_t numa =
          hwloc_get_ancestor_obj_by_type(topology_, HWLOC_OBJ_NUMANODE, pu);
      if (numa) {
  ...
  void InitializeWithSysfs() {
    ...
    for (int i = 0; i < num_cpus; ++i) {
      ...
      for (int n = 0; n < numa_node_count_; ++n) {
        char path[128];
        snprintf(path, sizeof(path), "/sys/devices/system/node/node%d/cpu%d", n,
                 i);
        if (access(path, F_OK) == 0) {
          info.numa_node = n;
          break;
