...
TEST(AsyncBackend, MatMulBasic) {
  ...
  // View creation
  auto A =
      TensorView::Make2D(data_A.data(), 2, 2, 2 * sizeof(float), DType::F32);
  auto B =
      TensorView::Make2D(data_B.data(), 2, 2, 2 * sizeof(float), DType::F32);
  auto C =
      TensorView::Make2D(data_C.data(), 2, 2, 2 * sizeof(float), DType::F32);
