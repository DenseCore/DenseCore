...
protected:
  void SetUp() override {
    manager = std::make_unique<BlockManager>(32, BLOCK_SIZE);
  }
...
TEST_F(BlockManagerTest, ComputeTokenHash) {
  ...
  uint64_t hash1 =
      BlockManager::ComputeTokenHash(tokens1.data(), tokens1.size());
  uint64_t hash2 =
      BlockManager::ComputeTokenHash(tokens2.data(), tokens2.size());
  uint64_t hash3 =
      BlockManager::ComputeTokenHash(tokens3.data(), tokens3.size());
