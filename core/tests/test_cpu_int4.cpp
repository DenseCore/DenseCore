 ...
 */
void GemvInt4_Reference(float* output, const float* input, const uint8_t* weights,
...
TEST_F(GemvInt4Test, AlignedK_128_GroupSize32) {
    ...
    // Compute reference
    GemvInt4_Reference(reference_.data(), input_.data(), weights_.data(), scales_.data(),
                       zeros_.data(), K, N, group_size, 0, N);

    // Compute using unified dispatch (AVX512/AVX2/Scalar)
    GemvInt4(output_.data(), input_.data(), weights_.data(), scales_.data(), zeros_.data(), K, N,
             group_size, 0, N);
